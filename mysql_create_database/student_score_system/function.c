...
void navigation()
{
	printf("\t\t\t************* MANAGEMENT SYSTEM ******************\n"
		   ...
		   "\t\t\t*        5 to   信息保存到数据库 . （保存）      *\n"
		   "\t\t\t*        6 to   将数据导出到指定文件 .           *\n"
		   "\t\t\t*        7.to   将数据库的文件输出到屏幕!（显示）*\n"
		   "\t\t\t*        8.to   将数据库中的文件写到缓冲区!(读取)*\n"
		   "\t\t\t*        9.to   数据排序                         *\n"
		   "\t\t\t*        10.to  插入学生信息 .                   *\n"
		   "\t\t\t*        11.to  格式化数据库                     *\n"
	       "\t\t\t*        12.to  格式化缓冲区                     *\n"
...
loop:	fputs("input student number(0001~9999),name,sex(0 is Man ,other is Woman):\n", stdout);
	scanf("%s%s%d", nptr->data.id, &nptr->data.name,  &nptr->data.sex);
	for (int i = 0; i < strlen(nptr->data.id); i++)
	{
		if (nptr->data.id[i] > 'A'&&nptr->data.id[i] < 'z')
		{
			printf("Warning !your input continue letter\n");
			goto loop;
		}
	}
